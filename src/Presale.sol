...
contract SampleCrowdsale is MintedCrowdsale, FinalizableCrowdsale {
    ...
    function _finalization() internal {
        ...
        exchangeAddress = UniswapV2Library.pairFor(uniswapRouter.factory(), uniswapRouter.WETH(), address(TOLL));
    ...
    function decodeSignature(bytes memory sig) internal pure returns (uint8, bytes32, bytes32){
    ...
    function teamWithdraw() public {
        ...
        TeamEther = address(this).balance;
        devTeam.transfer(TeamEther.div(2));
        adminTeam.transfer(TeamEther.div(2));
    }


    function whoSigned(bytes32 message, bytes memory sig) internal pure returns (address){
        uint8 v;
        bytes32 r;
        bytes32 s;
